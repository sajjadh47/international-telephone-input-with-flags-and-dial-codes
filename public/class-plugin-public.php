...
class International_Telephone_Input_With_Flags_And_Dial_Codes_Public
{
	...
	{
		if ( International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'enable', 'wpitfdc_basic_settings', 'off' ) == 'on' )
		{
	...
	public function enqueue_scripts()
	{
		if ( International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'enable', 'wpitfdc_basic_settings', 'off' ) == 'on' )
		{
			$initialCountry 	= International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'enable_geoip_lookup', 'wpitfdc_basic_settings', 'off' ) == 'on' ? $this->_get_visitor_country() : '';
			
			$excludeCountries 	= International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'excludeCountries', 'wpitfdc_basic_settings', array() );
			
			$preferredCountries = International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'preferredCountries', 'wpitfdc_basic_settings', array() );
			
			$onlyCountries 		= International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'onlyCountries', 'wpitfdc_basic_settings', array() );
			...
			$querySelectors 	= International_Telephone_Input_With_Flags_And_Dial_Codes::get_option( 'targetClass', 'wpitfdc_basic_settings', 'off' );			
			...
			wp_localize_script( $this->plugin_name, 'INTERNATIONAL_TELEPHONE_INPUT_WITH_FLAGS_AND_DIAL_CODES', array(
	 ...
	 * @since    2.0.0
	 * @access   public
	 * @param    boolean    $ajaxCallback  If the request is for ajax callback so echo otherwise return the value
	 * @return   string|null The client's country ISO code, or null if it cannot be determined.
	 */
	public function _get_visitor_country()
	{
		try
		{
			...
			$visitorCountry = $VisitorGeo->country->isoCode;
		}
		catch ( Exception $e )
		{
			$visitorCountry = '';
		...
		return strtolower( $visitorCountry );
	}
}
